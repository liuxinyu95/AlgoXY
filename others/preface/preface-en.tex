...
The \textproc{Unique-Enqueue} function takes $O(m)$ time to insert an unique element in ascending order, where $m = |Q|$ is the length of the queue. $m$ increases proportion to $n$ (Each time, we dequeue an element, and enqueue at most 3. The increasing ratio $\leq$ 2), the total time is $O(1 + 2 + 3 + ... + n) = O(n^2)$. \Cref{fig:big-O-1q} shows the number of queue access against $n$. It is a quadratic curve, which reflects the $O(n^2)$ performance.
...
We reach to the answer after dequeue $n$ smallest numbers from the three queues. \Cref{fig:q235} gives the first 4 steps.
...
\Question{For the free number puzzle, since all numbers are not negative, we can leverage the sign as a flag to indicate a number exists. Scan the list, for every number $|x| < n$ (where $n$ is the length), negate the number at position $|x|$. Then run another round of scan to find out the first positive number. Its position is the answer. Write a program to realize this solution.}
