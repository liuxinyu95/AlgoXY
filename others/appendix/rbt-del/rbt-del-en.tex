...
When fixing, we pass in the root $T$, the node $x$ (can be doubly black), and a flag $f$. The flag is true if $x$ is doubly black NIL. We record the doubly black NIL with $n$, and replace $n$ with the normal NIL after fixing.
