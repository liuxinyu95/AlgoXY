...
上面的C++模板元编程（tempalte meta programming）例子本质上是literate构建列表的形式。一个列表可以通过一个元素和一个子列表来构建。子列表可以为空。我们定义构造函数$cons(x, L)$。大量的Lisp方言都使用这一名称。在ML语言家族，‘cons’被定义为二元操作符\textt{::}，在Haskell中，对应的操作符为\texttt{:}。
