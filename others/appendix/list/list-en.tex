...
operation is expected to be $group(``Mississippi'') = \{ ``M'', ``i'', ``ss'', ``i'', ``ss'', ``i'', ``pp'', ``i''\}$.
...
We want to divide it into several small list, that eash sub-list is ordered either ascending or descending.
The grouping operation is expected to be $group(L) = \{ \{15, 9, 0\}, \{12, 11, 7\}, \{10, 5\}, \{6, 13\},
\{1, 4, 8\}, \{3, 14\}, \{2\}\}$.
...
However, this program performs in quadratic time if the appending function isn't optimized by store the tail position.
...
TODO: implement the two solutions for grouping ``mississippi'' and list of numbers

TODO: design the algorith by using span

TODO: equality properties
