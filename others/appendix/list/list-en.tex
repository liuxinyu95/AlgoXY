...
Every node links to the next one or NIL. Linked-list is often defined through compound data structure\footnote{In most cases, the data stored in list have the same type. However, there is also heterogeneous list, like the list in Lisp for example.}, for example:
...
\index{List!length}
As list is defined recursively, we can count the length of a given list in a recursive manner: for empty list, the length is zero, otherwise, it is the length of the sub-list plus one.
...
\begin{Exercise}
\Question{For list of type $A$, suppose we can test if any two elements $x, y \in A$ are equal, define a algorithm to test if two lists are identical to each other.}
\end{Exercise}
...
\begin{figure}[htbp]
    \centering
    \subcaptionbox{$p_2$ starts from the head, which is behind $p_1$ in $i$ steps.}{\includegraphics[scale=0.8]{img/list-rindex.ps}} \\
    \subcaptionbox{When $p_1$ reaches the end, $p_2$ points to the $i$-th element from right.}{\includegraphics[scale=0.8]{img/list-rindex-2.ps}}
