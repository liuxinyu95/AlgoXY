...
different representations. One is the trivial `NIL' (or null, 0 varies from languages);
...
Strictly speaking, we can't mutate the list at all in purely functional settings. What we mutate in
imperative settings is just creation of new list. Almost all functional environments support garbage
collection, the original list may either persisted for reusing, or released (dropped) at sometime \cite{okasaki-book}(Chapter 2).
...
If the list $L$ is sorted, that is for any position $0 \leq i \leq j \leq N$, we have $l_i \leq l_j$.
...
constraint can be loss to the strict less ($<$), that if elements can be compare in terms of $<$, we can design a program
to insert element so that the result list is still sorted. Readers can refer to the chapter of sorting for details about
ordering.
...
the semantic deletion means is actually create a 'new' list with all the elements in previous one except for
...
This is a linear time algorithm as well, and there are also other alternatives for implementation, for example, we can 
...
\label{concat}
Concatenation can be considered as a general case for appending, that appending only add one more extra element to the end of the list, while concatenation add multiple elements.

However, implementing concatenation naively by appending leads to quadratic algorithm which performs poor. Consider the
following equation.
...
It's neccessary to realize the inverse operation of zipping, that converts a list of pairs, to different
lists of elements. Back to the purchasing example, It's quite possible that the unit price information
is stored in a assocication list like $U = \{(apple, 1.00), (orange, 0.80), (banana, 10.05), ...\}$, so that
it's convenient to look up the price with a given product name, $lookup(melon, U)$. Similiarily, the
...
Altough the definition of $unzip$ can be directly written as the inverse of $zip$, here we give a realization based on
folding-right.
...
intermediate lists respectively. Note that we uses implict pattern matching to extract the elements from 
...
\item For the purchase payment example, suppose the quantity accosication list only contains those items with
the quantity isn't zero, that instead of a list of $Q = \{(apple, 3), (banana, 0), (orange, 1), ...\}$, it
hold a list like $Q = \{(apple, 3), (orange, 1), ...\}$. The `banana' infomation is filtered because the customer
...
is a kind of `hello world' topic, and refers to this appendix when meets any unfarmiliar list operations.
