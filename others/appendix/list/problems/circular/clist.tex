...
$|OA| = |AB|$. Denote the node where the circle start as the $k$-th one, the
...
It means that the meet point is the $k$-node before $A$ around the circle.
...
This solution is linear. The slow pointer exactly visits $n$ nodes when there is a circle.

The complete ANSI C example program with test are distributed with this article.
