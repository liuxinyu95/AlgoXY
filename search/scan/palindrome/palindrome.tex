...
This ensures the overall performace of generalised suffix tree method to be linear.
...
new palindrome talbe $P'$, the value corresponds to $s_i$ is $p'_j = 2k + 2$.
...
new palindrome talbe $P'$, the value corresponds to the new center is $p'_j = 2k + 1$.
\end{itemize}
...
one, the problem can be solved with the following
strategy:

\begin{enumerate}
\item Transform to a new string with speical delimiter interspersed;
...
in the palindrome table. Its length can be given by decreasing the maximum by one.
\end{enumerate}
...
succesfully with any $s_j$, $j < i$. The overal unsuccessful comparison is at most $n$.
