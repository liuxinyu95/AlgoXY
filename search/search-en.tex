...
by one. If the vote counter becomes negative, it means this candidate is voted out; We select
the next candidate as the new winner and repeate the above scanning process.

TODO: a figures to illustrate this step by step

The key point is that, if there exits majority greater or equal to 50\%, it can't be voted out
by all the others. However, if there are not any candidates win half of the votes, the recorded
`winner' is invalid. Thus it neccessary to perform a second round scan for verification.

The following pseudo code illustrates this algorithm.

\begin{algorithmic}[1]
\Function{Majority}{$A$}
  \State $x \gets A[1], c \gets 1$
  \For{$i \gets$ 2 to $|A|$}
    \If{$A[i] = x$}
      \State $c \gets c + 1$
    \Else
...
If there is the majority element, this algorithm take one pass to scan the votes. In every
iteration, it either increases or decreases the counter according to the vote is support
or against the current selection. If the counter becomes negative, it means the current
selection is voted out. So the new one is selected as the updated candiate and the 
counter is reset to one for further scan.
...
even there isn't. It's neccessary to scan the votes for the second round for verification.
...
majority(L) = \left \{
  ...
  {r@{\quad:\quad}l}
  fail & L = \Phi \\
  c & c = maj(l_1, 1, L'), |\{x | x \in L, x = c\}| \geq \%50 |L| \\
  fail & otherwise
  \end{array}
\right.
\ee
...
At any time when we scan to the $i$-th position, the max sum fond so far is recorded as $A$. At the same time, we
...
we reset it to 0. The following tables illustrated the steps when we can the example vector 
$\{3, -13, 19, -12, 1, 9, 18, -16, 15, -15\}$.

TODO: add a table to illustrate the steps.
