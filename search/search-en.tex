...
each column are in non-decreasing order. Figure \ref{fig:matrix-eg} illustrates such a matrix for example.
...
Given a value $x$, how to locate it in the matrix quickly? We need develop an algorithm, which returns
$(i, j)$ so that $m_{i,j} = x$.
...
Let's see how they got stucked.
