...
left are same, it fails if the last one does not, as shown in figure \ref{fig:bad-char}. What's more,
we wouldn't find a match even if we slide the pattern down by 1, nor 2. Actually, the length of the
pattern `ananym' is 5, the last one is `m' in the patter, however, the correcponding character in the
...
This leads to the {\em bad-character} rule. We can do a preprocessing for the pattern. If the character set
...
miss any matches, we have to slide down the pattern to check again. This is shown as in the figure \ref{fig:good-char}
...
It's quite possible that that unmatched character appears in the pattern more than one position. Denote the
...
There is a good result in practice, that only using the bad-character rule leads to a simple and fast string matching
algorithm, called Boyer-Moore-Horspool algorithm \cite{boyer-moore-horspool}.
...
The character set is denoted as $\Sigma$, we first intialize all the values of sliding function $\pi(c)$ as the length
of the pattern string $|P|$. After that we process the pattern from left to right, update the sliding function value.
...
the former case, we look up $\pi(c)$ to slide the pattern down to the right.
...
The algorithm firstly takes about $O(|\Sigma| + |P|)$ time to build the sliding function. If the character set size is
small, the performance is dominated by the pattern and the text. There is definitely a worst case
that all the characters in the pattern and text are same, e.g. searching `aa...a' in text `aa...a'. The performance in 
this worst case is $O(mn)$. This algorithm performs well if the pattern is long, and there is constant number of matchings.
This is as same as the bese case of full Boyer-Moore algorithm which will be explained next.
