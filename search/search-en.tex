...
Thus it takes at most $2(z+1)$ steps to complete searching. This is the worst case. The best case happens
that in every iteration, both $p$ and $q$ advance by one, this case need only $z+1$ steps.

Figure xxx illustrated the bese case and the worst case respectively.

Compare to the quadratic brute-froce method ($O(z^2)$), we improve to a linear algorithm bound to $O(z)$.

TODO: Performance analysis, provide figures.

TODO: Improve this algorithm with binary search for accurate boudaries
