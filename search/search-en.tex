...
A state is either empty $E$, or contains three part: the current state, the new state if match fails, and
...
\be
build((P_p, P_s), E, E) = \left \{
  \begin{array}
  {r@{\quad:\quad}l}
  build(\pi(P_p, P_s), E, E) & P_s = \Phi \\
...
\[
\begin{array}{l}
L = build(\pi(P_p, P_s), E, E) \\
R = build((P_s \cup \{p\}, P_s'), E, E))
\end{array}
\]
...
\begin{figure}[htbp]
 ...
 \caption{The infinity state tree for pattern `ananym'.}
 \label{fig:fallback-tree}
\end{figure}

Figure \ref{fig:fallback-tree} illustrates such an infinity state tree for pattern string `ananym'.
