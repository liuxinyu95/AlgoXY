...
\begin{figure}[htbp]
 \centering
 \subfloat[Case 1, Only a part of the matching suffix occurs as a prefix of the pattern.]{\includegraphics[scale=0.2]{img/good-suffix-case1.eps}}
...
For case 1, we can check every suffix of $P$, which includes $P_m$, $P_{(m-1, m)}$, $P_{(m-2, ..., m)}$, ... $P_{(i, ..., m)}$, ..., $P_{(2, ..., m)}$ to examine if it is the prefix of $P$. Where $P_{i, ..., j}$ means the sub-string
of $P$ from the $i$-th character to the $j$-th.
This can be achieved by a round of scan of the pattern from right to the left.

For case 2, we can check every prefix of $P$ includes $P_1$, $P_{(1, 2)}$, ..., $P_{(1, ..., m-1)}$ to examine if the longest suffix of it is also a suffix of $P$. This can be achieved by another
round of scan from the left to the right of the pattern.

\begin{algorithmic}
\Function{Good-Suffix}{$P$}
  \State $m \gets |P|$
  \State $S \gets \{0, 0, ..., 0\}$ \Comment{Initialize the table of length $m$}
  \State $l \gets 0$ \Comment{The last suffix which is also prefix of $P$}
  \For{$i \gets m-1$ down-to $1$} \Comment{first loop for case 1}
    \If{$P_{(i,...,m)} \sqsubset P$} \Comment{$\sqsubset$ means is prefix of}
      \State $l \gets i$
    \EndIf
    \State $S[i] \gets l$
  \EndFor
...
TODO: Realize the algorithms of good-suffix rule
