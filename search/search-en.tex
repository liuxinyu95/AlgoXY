...
bsearch(f, y, l, u) = \left \{
  ...
  {r@{\quad:\quad}l}
  l & l \geq u \\
  bsearch(f, y, m + 1, u) & f(m) \leq y, m = \lfloor \frac{l + u}{2} \rfloor \\
...
boundary is returned for the further saddleback search;
If the middle point produces a value less than or equal to the target, the lower bound is updated as the integer
next to the middle point; otherwise, the middle point becomes the new upperbound. The following
...
Eash round is bound to $O(\log z)$ times of calculation for $f$; After that, it takes $O(m + n)$
...
much less than $z$ ($m \ll z$, $n \ll z$). The performance is near to $O(\log z)$.

TODO: imperative realization.
