...
At any time, if we find there have been already bread crumbs left, it means we entered a loop, we
must go back and try different ways. Repeat these try-and-check steps, we can either find the way
out, or find the `no solution' fact. In the later case, we back-track to the start point.
