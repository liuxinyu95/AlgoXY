...
Most of the results are based on Chris, Okasaki's great
work in \cite{okasaki-book}. 
...
For a complete binary tree, if we define the depth as 0 if the tree with only has 1 leaf, 
we can we denote the tree as $t_i$ if its depth is $i+1$. It's obvious that there
are $2^i$ leaves in $t_i$.

For any sequence contains $N$ elements, it can be turned to a forest of compelte binary trees of this manner.
...
Let's formalize the algorithm to equations.
