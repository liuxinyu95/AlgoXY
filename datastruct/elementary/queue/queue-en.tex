...
The $abort$ function is only called during pop. When $n = 0$, the rotate just finished, is about to transform the state from $(S_c, 0, (x \cons a), f')$ to $(S_f, a)$. While $x$ being linked previously is the one to be popped, hence, we need remove $x$ and return $a$ as the result.
}
