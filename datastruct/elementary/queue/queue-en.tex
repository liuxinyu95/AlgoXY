...
single linked-list and circular buffer in first section; Then we
...
constant time, we need find real-time solution (or worst-time solution)
...
Some programming languages, such as Haskell, and most Object-oriented
...
queue properties. This is because single linked-list, which
...
takes time proportion to the length of the rear list. it's bound $O(\lg N)$
...
a solution mimic to the paired-lists queue: We concatenate two arrays, head-to-head, to
...
Both $F$ and the result of $reverse(R)$ are single linked-list, 
...
the $step()$ function to continuously appending the result to $F$
after $R$ is reversed.
...
\begin{Exercise}
\begin{itemize}
\item Why we need rollback one element when $n=0$ in $abort()$ function?
...
the use this lazy rotation as the new front list $F'$; the new rear
...
exhausted, and start new rotation accordingly; otherwise, it just consumes
...
approach revealed in this chapter. We we can handle elements effectively
...
\begin{Exercise}
\begin{itemize}
\item Realize dequeue, wich support add and remove elements on both sides in
