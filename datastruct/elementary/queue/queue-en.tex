...
\Function{Dequeue}{$Q$}
  ...
  \If{$x = $ \Call{Tail}{$Q$}} \Comment{$Q$Ø:z}
...
\begin{figure}[htbp]
 \centering
 \subcaptionbox{Before \textproc{Enqueue} $x$ to queue}{\includegraphics[scale=0.7]{img/enq-list-before}} \\
 \subcaptionbox{After \textproc{Enqueue} $x$ to queue}{\includegraphics[scale=0.7]{img/enq-list-after}} \\
 \subcaptionbox{Before \textproc{Dequeue}}{\includegraphics[scale=0.7]{img/deq-list-before}} \\
 \subcaptionbox{After \textproc{Dequeue}}{\includegraphics[scale=0.7]{img/deq-list-after}} \\
...
\begin{figure}[htbp]
 \centering
 \subcaptionbox{Continuously add some elements.}{\includegraphics[scale=0.7]{img/ring-buf-1}} \\
 \subcaptionbox{After remove some elements from head, there are free cells.}{\includegraphics[scale=0.7]{img/ring-buf-2}} \\
 \subcaptionbox{Go on adding elements till the boundary of the array.}{\includegraphics[scale=0.7]{img/ring-buf-3}} \\
 \subcaptionbox{The next element is added to the first free cell on head.}{\includegraphics[scale=0.7]{img/ring-buf-4}} \\
 \subcaptionbox{All cells are occupied. The queue is full.}{\includegraphics[scale=0.7]{img/ring-buf-5}}
 \caption{A queue is realized with ring buffer.} \label{fig:circular-buffer-queue}
\end{figure}

\begin{figure}[htbp]
 \centering
 \includegraphics[scale=0.5]{img/ring-buffer}
...
To realize \textproc{Enqueue} and \textproc{Dequeue}, an easy way
is to calculate the modular of index as the following.
...
Key headQ(struct Queue* q) {
    return q->buf[q->head];  //¼ÙÉè¶ÓÁÐ²»Îª¿Õ¡£
}
...
test the queue is empty or full with only head and tail pointers?
Note that the head can be either before or after the tail.
\end{Exercise}
...
\begin{figure}[htbp]
  \centering
  \subcaptionbox{\textproc{DeQueue} performs poorly.}{\includegraphics[scale=0.5]{img/list-queue-head}} \\
  \subcaptionbox{\textproc{EnQueue} performs poorly.}{\includegraphics[scale=0.5]{img/list-queue-tail}}
...
\begin{figure}[htbp]
  \centering
  \subcaptionbox{a horseshoe magnet.}{\includegraphics[scale=0.5]{img/horseshoe-magnet}} \\
  \subcaptionbox{concatenate two lists tail-to-tail.}{\includegraphics[scale=0.5]{img/front-rear-queue}}
...
\begin{figure}[htbp]
  \centering
  \subcaptionbox{a horseshoe magnet.}{\includegraphics[scale=0.5]{img/horseshoe-magnet}}
  \subcaptionbox{concatenate two arrays head-to-head.}{\includegraphics[scale=0.5]{img/front-rear-array-queue}}
