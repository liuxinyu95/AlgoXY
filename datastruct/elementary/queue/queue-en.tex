...
would make no sense. Actually, we can perform a `lazy' copying, that the real copying
...
\item In the footnote, we mentioned that when we start incremental $F \cup reverse(R)$, 
copying the front list can't be done monolithic. Implement the lazy copying
so that we copy one element per step along with the reversing. Since Haskell
is lazy language, please do this in some other functional programming language,
for instance, Scheme/Lisp, which enable explicitly control of laziness.
