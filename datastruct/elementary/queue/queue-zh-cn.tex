...
\be
\begin{array}{rcll}
next\ (S_r, n, x:f, f', y:r, r') & = & (S_r, n + 1, f, x:f', r, y:r') & \text{同时反转}f, r\\
next\ (S_r, n, [\ ], f', [y], r') & = & (S_c, n, f', y:r') & \text{反转结束、转入连接}\\
next\ (S_c, 0, a, [\ ]) & = & (S_f, a) & \text{连接结束}\\
next\ (S_c, n, a, x:f') & = & (S_c, n - 1, x:a, f') & \text{逐步连接}\\
\end{array}
\ee

接下来我们需要将这些递进的步骤分配到每个出、入队操作中以实现一个实时队列。
