...
next\ (S_r, f', [\ ], r', [y]) & = & next\ (S_c, f', y:r') & \text{反转结束、转入连接}\\
...
next\ (S_r, n, f', [\ ], r', [y]) & = & next\ (S_c, n, f', y:r') & \text{反转结束、转入连接}\\
...
\begin{Exercise}
\Question{在$abort$函数中，当$n = 0$时，当$n = 0$时，我们实际上撤销了上一个链接元素的操作，去掉了$x$而返回$a$作为结果。为什么需要回滚一个元素？}
...
我们将$xs$初始化为$f$列表，$ys$初始化为$r$列表，$a$初始化为空$[\ ]$。当平衡条件不满足$|f| + 1 = |r|$时开始进行轮转。在轮转过程中，我们始终保持$|xs| + 1 = |ys|$成立。为了推导轮转的实现，我们先考虑最简单的情况：
...
\be
  \begin{array}{rcll}
  rotate\ (x \cons xs)\ (y \cons ys)\ a & = & (x \cons xs) \doubleplus (reverse\ (y \cons ys)) \doubleplus a & \text{定义(\ref{eq:rot-def})} \\
  & = & x : (xs \doubleplus reverse\ (y \cons ys)) \doubleplus a) & \text{连接的结合性} \\
  & = &  x : (xs \doubleplus reverse\ ys \doubleplus (y \cons a)) & \text{反转的性质和连接的结合性} \\
  & = & x : (rotate\ xs\ ys\ (y \cons a)） & \text{反向用定义(\ref{eq:rot-def})}
  \end{array}
\ee
...
其中每次$balance$操作都会向前推进一次轮转计算，当轮转结束时，我们开始另一轮轮转：
