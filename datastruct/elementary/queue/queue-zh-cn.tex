...
\textproc{Head}仅按照FIFO的顺序获取下一个元素而不会将元素删除，而\textproc{Dequeue}会执行删除操作。这些操作也分别称作\textit{push}、\textit{pop}、\textit{front}\footnote{enqueue也称作snoc, append, push back；dequeue也称作pop, pop front。}。使用列表时，我们选择在头部加入元素、从尾部删除元素以简化实现。
...
\begin{figure}[htbp]
  \centering
  \subcaptionbox{\textproc{Enqueue}前}{\includegraphics[scale=0.7]{img/enq-list-before}} \\
  \subcaptionbox{\textproc{Enqueue}后}{\includegraphics[scale=0.7]{img/enq-list-after}} \\
  \subcaptionbox{\textproc{Dequeue}前}{\includegraphics[scale=0.7]{img/deq-list-before}} \\
  \subcaptionbox{\textproc{Dequeue}后}{\includegraphics[scale=0.7]{img/deq-list-after}} \\
  \caption{列表实现的\textproc{Enqueue}、\textproc{Dequeue}}
...
和列表相反，我们可以在常数时间将元素添加到数组末尾，但需要线性时间$O(n)$从头部删除。这是因为要将全部剩余元素依次向前移动。为了达到队列的性能要求，我们可以把数组的头尾连接起来，做成一个环，叫做循环缓冲区，如图如图\ref{fig:circular-buffer}、\ref{fig:circular-buffer-queue}所示。这样用数组的头部坐标head，队列长度count，和数组大小size，就可以完全表述队列。count等于0时队列为空。利用模运算简化入队、出队的实现。
...
\begin{Exercise}
循环缓冲区在初始化时规定了最大的容量，如果使用头、尾两个指针，而不使用Count，如何检测队列是否为空？是否已满？（考虑两种情况：头部在尾部前面，和头部在尾部后面）。
\end{Exercise}
...
\begin{figure}[htbp]
  \centering
  \subcaptionbox{\textproc{DeQueue}为线性时间。}{\includegraphics[scale=0.5]{img/list-queue-head}} \\
  \subcaptionbox{\textproc{EnQueue}为线性时间。}{\includegraphics[scale=0.5]{img/list-queue-tail}}
...
\begin{figure}[htbp]
  \centering
  \subcaptionbox{马蹄形磁铁}{\includegraphics[scale=0.5]{img/horseshoe-magnet}} \\
  \subcaptionbox{“尾对尾”接在一起的列表}{\includegraphics[scale=0.5]{img/front-rear-queue}}
