...
堆是一种常见的数据结构，可以用于排序、带有优先级的调度、图算法等\cite{wiki-heap}。堆通常用数组表示的二叉树\cite{CLRS}实现。Floyd由此给出了一种高效的堆排序算法\cite{wiki-heapsort}\cite{rosetta-heapsort}。堆本身的定义是抽象的，也可由其它数据结构实现。本章介绍用二叉树实现的堆，包括左偏堆（Leftist heap）、斜堆（Skew heap）、伸展堆（splay heap）\cite{okasaki-book}。一个堆或为空，或包含若干可比较的元素。它满足一条性质并定义了如下操作：
...
Floyd给出了另一种实现。构造一个大顶堆。将堆顶和数组末尾交换，这样最大元素就存储到了排序后的正确位置。而原来的末尾元素变成了新的堆顶。将堆的大小减一，然后执行\textproc{Heapify}恢复堆性质。重复这一过程，直到堆中仅剩一个元素。这一算法省去了额外的空间。
...
传入两棵子树$A$、$B$和元素$k$。若$A$的秩较小，则用$B$作为左子树、$A$作为右子树。新节点的秩为$1 + rank(A)$；否则，若$B$的秩较小，就用$A$作为左子树，$B$作为右子树。新节点的秩为$1 + rank(B)$。给定两个左偏堆$H_1$和$H_2$，它们不空时分别记为$(r_1, L_1, K_1, R_1)$、$(r_2, L_2, k_2, R_2)$，下面的函数定义了合并操作：
...
当合并两棵非空斜树时，先比较根节点，选择较小的作为新的根。然后把含有较大元素的树合并到某一子树上。最后再把左右子树交换。令两棵非空子树为：$H_1 = (L_1, k_1, R_1)$、$H_2 =(L_2, k_2, R_2)$。若$k_1 < k_2$，选择$k_1$作为新的根。我们既可以将$H_2$和$L_1$合并，也可以将$H_2$和$R_1$合并。不失一般性，我们合并到$R_1$上。然后交换左右子树，最后的结果为$(\textit{merge}\ R_1\ H_2, k_1, L_1)$。
...
\begin{enumerate}
\item \textbf{zig-zig}步骤，$x$和$p$都是左子树或者$x$和$p$都是右子树。我们通过两次旋转，将$x$变成根节点。

\item \textbf{zig-zag}步骤，$x$和$p$一棵是左子树另一棵是右子树。经过旋转，$x$变成根节点，$p$和$g$变成了兄弟节点。

\item \textbf{zig}步骤，这种情况下，$p$是根节点，经过旋转，$x$变成了根节点。这是伸展操作的最后一步。
\end{enumerate}

\begin{figure}[htbp]
  \centering
  \includegraphics[scale=0.45]{img/splay}
  \caption{\textbf{zig-zig}: $x$和$p$都是左子树或都是右子树，$x$成为根节点。\textbf{zig-zag}: $x$和$p$一棵是左子树另一棵是右子树，$x$成为根节点，$p$和$g$变成了兄弟节点。\textbf{zig}: $p$是根节点，旋转后$x$变为根节点。}
  \label{fig:splay}
\end{figure}
...
斜堆的实现更简单，我们无需维护子树的秩，省去了回溯的过程，因此无需父节点引用：
\begin{Bourbaki}
data Node<T> {
    T value
