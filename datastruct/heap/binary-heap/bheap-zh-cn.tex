...
虽然\textproc{Heapify}算法的复杂度为$O(\lg n)$，但是\textproc{Build-Heap}的复杂度不是$O(n \lg n)$，而是线性时间$O(n)$的。我们跳过了所有的叶子节点，最多有一半的节点被比较并向下移动一次；最多有$1/4$的节点被比较并向下移动两次；最多有$1/8$的节点被比较并向下移动三次……总共比较和移动次数的上限为：
