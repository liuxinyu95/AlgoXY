...
\begin{enumerate}
\item 顶部总保存着最小（或最大）元素；
\item 弹出操作移除顶部元素，并保持堆的性质：新的顶部仍是剩余元素中最小（或最大）的；
\item 将新元素插入到堆中仍然保持性质1；
\item 其它操作（例如合并两个堆）都保持性质1。
\end{enumerate}

我们称顶部保存最小元素的堆为\textbf{最小堆}，顶部保存最大元素的堆为\textbf{最大堆}。
