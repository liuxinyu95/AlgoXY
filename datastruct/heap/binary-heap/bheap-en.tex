...
we call the splay function to adjut the tree.
...
which donwgrade to linked-list with normal binary search tree.
...
Whenever we follow
two left branches, or two right branches continously, we rotate
the two nodes.
...
If $k < p$, there are two sub-cases. one is trival case that
...
partiton $R'$ with the pivot, all the elements less than $p$ in
...
The corresponding Haskell example program is as the folloing.
...
Note that the third clause performs splaying without explictly call
the $partition$ function. It utilizes the properity of binary
...
denote the first splay heap as $(L, k, R)$, we parition $T_2$ with $k$ as the
pivot to yield $(A, B)$, where $A$ contains all the elements in $T_2$ which are
less than $k$, and $B$ holds the rest. We next recurisvely merge $A$ with $L$;
and merge $B$ with $R$ as the new children for $T_1$.

Translating the defintion to Haskell gives the following example program.
