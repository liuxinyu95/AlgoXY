...
void heapify(Array& a, unsigned i, unsigned n, LessOp lt){
  while(true){
    unsigned l=left(i);
    unsigned r=right(i);
    unsigned smallest=i;
...
\begin{lstlisting}
template<class Array, class LessOp>
void build_heap(Array& a, unsigned n, LessOp lt){
  unsigned i = (n-1)>>1;
  do {
      heapify(a, i, n, lt);
  } while (i--);
...
example C++ program implements this algorithm.

\lstset{language=C++}
\begin{lstlisting}
template<class E, class T, class LessOp>
E heap_pop(T& a, unsigned& n, LessOp lt){
  E top = heap_top(a);
  a[0] = a[n-1];
  heapify(a, 0, --n, lt);
  return top;
...
Below example C++ program implements the top-$k$ algorithm.

\lstset{language=C++}
\begin{lstlisting}
template<class Iter, class Array, class LessOp>
void heap_top_k(Iter res, unsigned k,
                Array& a, unsigned& n, LessOp lt){
  build_heap(a, n, lt);
  unsigned i, count = min(k, n);
  for(i=0; i<count; ++i)
    *res++=heap_pop(a, n, lt);
}
\end{lstlisting}
