  ...
  \While{\Call{Parent}{$x$} $\neq NIL$ and \textproc{Color}(\Call{Parent}{$x$}) $= RED$}
    ...
    \Else
      ...
      \If{\Call{Parent}{$x$} = \textproc{Left}(\Call{GrandParent}{$x$})}
        ...
        \State \textproc{Color}(\Call{Parent}{$x$}) $\gets BLACK$
        \State \textproc{Color}(\Call{GrandParent}{$x$}) $\gets RED$
        \State $T \gets$ \textproc{Right-Rotate}($T$, \Call{GrandParent}{$x$})
      \Else
        ...
        \State \textproc{Color}(\Call{Parent}{$x$}) $\gets BLACK$
        \State \textproc{Color}(\Call{GrandParent}{$x$}) $\gets RED$
        \State $T \gets$ \textproc{Left-Rotate}($T$, \Call{GrandParent}{$x$})
      \EndIf
    \EndIf
  \EndWhile
...
Figure \ref{fig:imperative-insert} shows a result of repeatedly 
feed keys 1, 2, ..., 8 to the above python insertion program. Compare it with
figure \ref{fig:insert-example}, one can tell the difference clearly.

\begin{figure}[htbp]
       \centering
	\includegraphics[scale=0.5]{img/python-insert.ps}
        \caption{Repeatedly insert 1,2, ..., 8 to a red-black tree} 
        \label{fig:imperative-insert}
\end{figure}

We skip the red-black tree delete algorithm in imperative settings, becasue it
is even more complex than the insertion. The implementation of deleting is
left as an exercise of this chpater.

\subsection*{Exercise}
\begin{itemize}
\item Implement the red-black tree deleting algorithm in your favorate
