...
我们把每个值和一个标记存入节点中，树的类型为：$Tree\ (K, Bool)$，插入$x$时，使用$insert\ (x, True)$将节点标记为存在。删除时将标记变为False。然后统计已删除节点个数，并启动重建。
...
rebuild\ t = \begin{cases}
  cnt\ t > \dfrac{1}{2} (size\ t): & (fromList \circ (filter\ 2nd) \circ toList)\ t \\
  \text{否则}: & t
\end{cases}
\]

为了方便，我们可以在节点中缓存子树的大小：
