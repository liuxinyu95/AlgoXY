...
我们前面定义了递归插入的结果为一对值，例如$(L', \Delta H_l)$，我们需要对插入的结果调整平衡，并更新高度的增加值。函数$tree()$被定义用来完整这一任务。它接受4个参数：$(L', \Delta H_l)$、$k'$、$(R', \Delta H_r)$和$\Delta$。这一函数的运算结果记为$(T', \Delta H)$。其中，$T'$为调整平衡后的树，$\Delta H$是树高度的增加值，定义如下：
