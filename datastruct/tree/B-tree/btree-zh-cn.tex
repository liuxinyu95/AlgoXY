...
一棵二叉搜索树或者为空，或者包含一个键$k$和左右分枝$l$、$r$。左子树$l$中的任何键都小于$k$，并且$k$小于右子树$r$中的任何键：
...
B树推广到多个键和分枝。一棵B树或者为空，括者包含$n$个键和$n + 1$个子分枝，每个分枝也都是一棵B树。记这些键为$k_1, k_2, ..., k_n$，分枝为$t_1, t_2, ..., t_n, t_{n+1}$，如图\ref{fig:btree-node}所示：
...
\begin{itemize}
\item 键是递增的：$k_1 \leq k_2 \leq ... \leq k_n$；
\item 对于任意$k_i$，子树$t_i$中的所有键都小于$k_i$，并且$k_i$小于子树$t_{i+1}$的任意键。
\end{itemize}
...
叶子节点不包含子分枝。B树节点中可以保存每个键对应的值，但这不是必须的。简单起见，本章忽略了节点中的值。令键的类型为$K$，则B树的类型为$BTree\ K$或\texttt{BTree<K>}。此外，我们还需定义一组规则以保持B树平衡：
