...
The other way is to merge the nodes before delete if there are too few keys. Consider delete key $x$ from the tree, let us start from the easy case.

\textbf{Case 1}. If $x$ exists in node $t$, and $t$ is leaf, we can directly remove $x$ from $t$. If $t$ is the singleton node in the tree (root), we needn't worry about too few keys.

\textbf{Case 2}. Otherwise, we recursively lookup $x$ from the root. When reach to a node $t$ contains $x$, there are three sub-cases if $t$ contains insufficient keys ($d - 1$ keys include $x$):

\textbf{Case 2a}. As shown in \ref{fig:btree-del}, let the predecessor of $k_i = x$ be $k'$, where $k' = max(t_i)$. If $t_i$ has sufficient keys ($\geq d$), we replace $k$ with $k'$, then recursively delete $k'$ from $t_i$.

\textbf{Case 2b}. If $t_i$ does not have enough keys, but the sub-tree $t_{i+1}$ does ($\geq d$). Symmetrically, we replace $k$ with its successor $k''$, where $k'' = min(t_{i+1})$, then recursively delete $k''$ from $t_{i+1}$, as shown in \ref{fig:btree-del-case2b}.
...
\textbf{Case 2c}. Otherwise, if neither $y$, nor $z$ contains enough keys, we
