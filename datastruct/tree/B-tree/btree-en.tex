...
On top of it, we also need define a set of self-balance rules:
...
This is the reason why a B-tree is balanced. The simplest B-tree
is a so called 2-3-4 tree, where $t=2$, in that every node except
the root contains 2 or 3 or 4 keys. Essentially, a red-black tree can be mapped
to 2-3-4 tree.

The following Python code shows an example B-tree definition.
It explicitly accepts $t$ as a parameter when creating a node.
...
In this chapter, we will first introduce how to generate a B-tree by insertion.
...
keys from B-trees and how to look up a key.
