...
In previous chapter when developed integer prefix tree, we encoded the information in the edge of the binary tree. Another way to extend the binary search tree is to increase the number of the sub-trees from 2 to $k$. B-tree is such a data structure, that can be considered as a generic form of $k$-ary search tree. It is also developed to be self-balanced\cite{wiki-b-tree}. B-tree is widely in computer file system (some are based on B+ trees, an extension of B-tree) and database system. B-trees are also widely used in database systems. Figure \ref{fig:btree-example} gives an example B-tree, we can find the differences and similarities between B-tree and binary search tree.
...
A binary search tree is either empty or a node containing a key $k$ and two sub-trees $l$ and $r$. Every key in $l$ are less than $k$, while $k$ is less than every key in $r$:

\be
\forall\ \text{key}\ x \in l, \forall\ \text{key}\ y \in r \\
\Rightarrow x < k < y
\ee

Extend it to multiple keys and sub-trees, we obtained the B-tree. A B-tree is

\begin{itemize}
\item either empty;
\item or containing $n$ keys, and $n + 1$ sub-trees, each sub-tree is also a B-Tree. We denote these keys and sub-trees as $k_1, k_2, ..., k_n$ and $c_1, c_2, ..., c_n, c_{n+1}$, as shown in figure \ref{fig:btree-node}.
\end{itemize}
...
\item For every key $k_i$, all keys in sub-tree $c_i$ are less than it, while $k_i$ is less than every keys in sub-tree $c_{i+1}$:
\end{itemize}

\begin{equation}
\forall\ \text{key}\ x_i \in c_i, i=0, 1, ..., n, \Rightarrow x_1 < k_1 < x_2 < k_2 < ... < x_n < k_n < x_{n+1}
\label{eq:btree-order}
\end{equation}

There could be optional value stored in a B-tree node. We ignore it and denote the B-tree type as $BTree\ K$, or \texttt{BTree<K>}, where $K$ is the type of the key. On top of it, we also need define a set of self-balance rules:

\begin{enumerate}
\item All leaves have the same depth;
\item Let $t$ be the {\em minimum degree} number of a B-tree, such that each node:
  \begin{itemize}
  \item has at most $2t-1$ keys;
  \item has at least $t-1$ keys, except for the root;
  \end{itemize}
\end{enumerate}

\begin{proof}
Let's prove that a B-tree is always balanced. Consider a B-tree of $n$ keys. The minimum degree $t \geq 2$. Let the height be $h$. All the nodes have at least $t - 1$ keys except for the root. The root contains at least 1 key. There are at least 2 nodes at depth 1, at least $2t$ nodes at depth 2, at least $2t^2$ nodes at depth 3, ..., at least $2t^{h-1}$ nodes at depth $h$. Multiply all nodes with $t-1$ except for the root, the total number of keys satisfies the following:
...
Hence a B-tree is balanced. The simplest B-tree is called 2-3-4 tree, where $t = 2$. Every node except for the root contains 2, 3, or 4 keys. Essentially, a red-black tree can be mapped to a 2-3-4 tree. For a none empty B-tree node, we denote it as $(ks, ts, t)$, where $ks$ is the keys, $ts$ is the sub-trees, and $t$ is the minimum degree. Below example program defines the B-tree.
...
The empty node of degree $t$ is in the form of $(\nil, \nil, t)$ or \texttt{BTree [] [] t}.
...
\begin{figure}[htbp]
  \centering
  \subcaptionbox{Insert key 22 to the 2-3-4 tree. $22 > 20$, go to the right child; $22 < 26$ go to the first child.}{\includegraphics[scale=0.5]{img/btree-insert-simple1.ps}} \\
  \subcaptionbox{$21 < 22 < 25$, and the leaf isn't full.}{\includegraphics[scale=0.5]{img/btree-insert-simple2.ps}}
...
\begin{figure}[htbp]
  \centering
  \subcaptionbox{Before split}{\includegraphics[scale=0.5]{img/split-node-before.ps}} \\
  \subcaptionbox{After split}{\includegraphics[scale=0.5]{img/split-node-after.ps}}
  \caption{Split node}
...
\begin{figure}[htbp]
  \centering
  \subcaptionbox{Locate the child to insert.}{\includegraphics[scale=0.5]{img/insert-before.ps}} \\
  \subcaptionbox{Recursive insert.}{\includegraphics[scale=0.5]{img/insert-after.ps}}
...
\begin{figure}[htbp]
  \centering
  \subcaptionbox{Insert result of a 2-3-4 tree.}{\includegraphics[scale=0.5]{img/btree-insert-fp-234.ps}} \\
  \subcaptionbox{Insert result of a B-tree with $t = 3$}{\includegraphics[scale=0.5]{img/btree-insert-fp-3.ps}}
