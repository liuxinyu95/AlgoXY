...
We can use $\leq$ to allow duplicated keys in B-tree: all left ones are $\leq x$, while $x \leq$ all right ones. However, we need additional logic to handle duplicated keys when look up and delete a key. Typically, we constraint the key to be unique, while a key can map to multiple values: $k \mapsto [v_1, v_2, ...]$, called {\em multimap}.
}
