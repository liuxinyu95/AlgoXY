...
\Cref{fig:btree-insert-fp} shows the example B-trees built from ``GMPXACDEJKNORSTUVYZ''.
...
We can use $\leq$ to allow duplicated keys in B-tree: all left ones are $\leq x$, while $x \leq$ all right ones. However, we need additional logic to handle duplicated keys when lookup and delete a key. Typically, we constraint the key to be unique, while a key can map to multiple values: $k \mapsto [v_1, v_2, ...]$, called {\em multi-map}.
}
...
The performance is still linear. Although the binary search speed up to $O(\lg n)$, the insertion takes $O(n)$ time to shift elements.
}
\end{Answer}
...
\begin{Exercise}\label{ex:btree-binary_lookup}
  \Question{Improve the imperative lookup with binary search among keys.}
\end{Exercise}
...
\begin{Exercise}\label{ex:btree-del}
...
When partition the tree with $x$, we need use $\leq$, but not $<$, because $x$ may be the last one on the left. We abstract the partition predicate as a parameter:
