...
Function \texttt{isEmpty} tests if a tree is empty. The algorithm firstly look up the node to be deleted, then execute the deletion. This algorithm takes $O(h)$ time where $h$ is the height of the tree. We can also pass the node to be deleted directly to avoid looking up.
