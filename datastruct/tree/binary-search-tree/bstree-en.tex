...
Figure \ref{fig:bst-example} shows an example of BST. Comparing with
...
data structure is a linked list. Figure \ref{fig:lisp-layout} shows how a binary
...
In more functional settings, it's hard to use pointers for backtracking (and typically, there is no need for backtracking, since there are usuarly top-down recursive solutions), and so the `parent' field has been omitted in that layout.
...
There is an exceptional case that if $k$ is equal to the key of root, it means it already exists, and we can either overwrite the data, or just do nothing.
To simplify things, this case has been skipped in this section.
...
This algorithm can be expressed imperitively using iteration, completely free of recursion.
