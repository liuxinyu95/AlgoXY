...
\index{binary tree}
The map is a binary search tree. Here we use the word as the key, and its occurrence number as the value. This program runs fast, which reflects the power of binary search tree. Let us firstly define the binary tree. A binary tree can be defined recursively. It is either empty; or contains 3 parts: the element, and two sub-trees called left and right children. Figure \ref{fig:binary-tree-example} shows an example.
...
A binary search tree is a special binary tree that its elements are comparable\footnote{It is abstract ordering, not limit to magnitude, but like precedence, subset of etc. the `less than' (<) is abstract in this chapter.}, and satisfies the following constraints: (1) For any node, all the keys in its left sub-tree are less than the key in this node; (2) the key in this node is less than any key in its right sub-tree. Figure \ref{fig:bst-example} shows an example of binary search tree. Comparing with Figure \ref{fig:binary-tree-example}, we can see the differences in keys ordering. To highlight the elements in binary search tree is comparable, we call it as {\em key}, and name the augmented satellite data as {\em value}.
