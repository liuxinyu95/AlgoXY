...
We start from Binary Search Trees (BST) as the `hello world' data structure. Let us see an interesting programming problem given by Bentley in {\em Programming Pearls}\cite{Bentley}. It is about to count the number of words in text. One example solution is:\index{word counter}
...
We can run it to count the words like this:
...
The map provided in library is a kind of balanced BST. Here we use the word as the key, and its occurrence number as the value. This program runs fast, which reflects the power of BST. Prior to BST, let us first see the more generic tree, the binary tree. A binary tree can be defined recursively:
...
\begin{itemize}
\item either empty;
\item or contains 3 parts: the value, two sub-trees called left and right children.
\end{itemize}
...
\begin{figure}[htbp]
       \begin{center}
        ...
        \caption{A BST} \label{fig:bst-example}
       \end{center}
\end{figure}
