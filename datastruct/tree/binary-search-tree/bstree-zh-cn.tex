...
我们据此可以得到一个排序的方法：先把一个无序的列表转化为一个二叉搜索树，然后再用中序遍历把树转换回列表。该方法被称为“树排序”。

\be
  sort\ X = toList\ (fromList\ X)
\ee

或写成函数组合\cite{func-composition}的形式：
...
\subsection{查找}
二叉搜索树的递归特性非常适合自顶向下的查找。考虑在类型为$Tree\ (K, V)$的二叉搜索树中查找某个键$x$对应的值：
