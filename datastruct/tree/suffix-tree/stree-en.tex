...
\Function{Insert}{$top, c$}
  ...
  \EndIf
  \State $node \gets top$
  \State $node' \gets$ \Call{Create-Empty-Node}{} \Comment{dummy init value}
  \While{$node \neq$ NIL $\land$ \Call{Children}{$node$}[$c$] = NIL}
    \State \Call{Children}{$node$}[$c$] $\gets$ \Call{Create-Empty-Node}{}
    \State \Call{Suffix-Link}{$node'$} $\gets$ \Call{Children}{$node$}[$c$]
    \State $node' \gets$ \Call{Children}{$node$}[$c$]
    \State $node \gets$ \Call{Suffix-Link}{$node$}
  \EndWhile
  \If{$node \neq$ NIL}
    \State \Call{Suffix-Link}{$node'$} $\gets$ \Call{Children}{$node$}[$c$]
  \EndIf
