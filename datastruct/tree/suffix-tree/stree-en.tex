...
from the root to a leaf. Figure \cref{fig:stree-banana} shows the suffix tree for
English word `banana'.
...
suffix tree for 'bananas', as shown in figure \cref{fig:stree-bananas}, is quite different.
...
structure than suffix tree. Figure \cref{fig:strie-banana} shows the suffix trie for
'banana'.
...
Compare with figure \cref{fig:stree-banana}, we can find the difference between
...
\cref{fig:strie-cacao}, node $X$ represents suffix 'a', by adding character 'c',
...
Suffix link is drawn in dotted style. In figure \cref{fig:strie-cacao}, the suffix link of
...
table \cref{tab:suffixes_s_i}. There are total $i+1$ suffixes.
...
For example, in figure \cref{fig:strie-cac}, node $X$ and $Y$ are corresponding
...
If check the nodes one by one according to table \cref{tab:suffixes_s_i}, we can
...
According to this fact, we can refine the algorithm \cref{algo:strie1} to
the following.
...
Figure \cref{fig:cons-strie-cacao} shows the steps
...
Figure \cref{fig:stree-bananas-label} shows the suffix tree of English word ``bananas''.
...
For example, node $Y$ in Figure \cref{fig:stree-bananas-label} can be either
...
In \cref{ap-and-ep}, we mentioned, all updating to leaves is trivial, because we
...
This is illustrated in figure \cref{fig:implicit-end-point}. We can
...
$SuffixTree(S_{i+1})$. Figure \cref{fig:ep-ap} shows about this truth.
...
Figure \cref{fig:cons-stree-cacao} shows the steps when constructing the
...
Consider the example suffix tree shown in figure \cref{fig:stree-mississippis}
