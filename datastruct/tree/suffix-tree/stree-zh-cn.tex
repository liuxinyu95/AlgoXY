...
注意：这一算法的返回值不是根节点，而是后缀trie的top节点。我们可以通过沿着后缀链接遍历以获得根节点。
...
图\ref{fig:cons-strie-cacao}给出了构造字符串“cacao”后缀树的步骤。简单起见，我们只画出了最后的一组后缀链接。
...
由于每次沿着后缀链接遍历，算法\textproc{Insert}的时间复杂度和后缀trie的大小成正比。最坏情况下，对于长度为$n$的字符串，需要$O(n^2)$时间来构造后缀trie。例如字符串$S=a^nb^n$，有$n$个字符$a$和$n$个字符$b$，就可使得算法的性能严重下降。
...
算法接受一个引用对$(node, (l, i))$作为参数。这里$(node, (l, i-1)$是后缀树$SuffixTree(S_{i-1})$的active point。算法不断沿着后缀链接处理节点，直到位置$(node, (l, i-1))$成为end point。否则，函数\textproc{End-Point-Branch?}返回一个用于分支出新的叶子节点的位置。这个的实现如下：
