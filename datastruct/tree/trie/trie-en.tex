...
For non-empty chidren, $C = \{(k_1, T_1), (k_2, T_2), ..., (k_m, T_m)\}$,
...
In the lasy evaluation environment, the top $n$ candidates can be
...
Firsly, we need define the T9 mapping, which maps from digit to candidate
characers.
...
Suppose user input digits $K = k_1k_2...k_n$, If $K$ isn't empty, denote the
rest digits except for $k_1$ as $K'$,
below pseudo code shows how to realize T9 with trie.

\begin{algorithmic}[1]
\Function{Look-Up-T9}{$T, K$}
  \State $Q \gets \{(\Phi, K, T)\}$
  \State $R \gets \Phi$
  \While{$Q \neq \Phi$}
    \State $(prefix, K, T) \gets$ \Call{Pop}{$Q$}
    \For{each $c$ in $M_{T9}[k_1]$}
      \If{$c \in $ \Call{Children}{$T$}}
        \If{$K' = \Phi$}
          \State $R \gets R \cup \{prefix + c\}$
        \Else
          \State \textproc{Push}($Q, prefix + c, K', $ \Call{Children}{$t$})$[c]$)
        \EndIf
      \EndIf
    \EndFor
  \EndWhile
...
intialized with a tuple $(prefix, K, T)$, containing empty prefix, the digit sequence to be
...
using the rest of digits to update $K$, and use that sub-tree. This new tuple
...
the program creats a new tuple, updates the prefix, the digits to be processed,
...
\Function{Look-Up-T9}{$T, K$}
  ...
  \While{$Q \neq \Phi$}
    ...
    \For{each $(k_i, T_i) \in $ \Call{Children}{$T$}}
      ...
      \If{$D \sqsubset K$} \Comment{$D$ is prefix of $K$}
        \If{$D = K$}
          \State $R \gets R \cup \{prefix + k_i\}$
        \Else
          \State \textproc{Push}($Q, prefix + k_i, K - D, T_i$)
        \EndIf
      \EndIf
    \EndFor
  \EndWhile
...
natural extensions. They can be used to manipulate text infromation.
As examples, predictive e-dictionary and a T9 input method are
realized with trie or Patricia. Although these examples
are different from the real implementation in commercial software.
They show simple approaches to solve some problems. There are still some
interesting problems can not be solved with trie or Patrica directly,
however, Other data structures, such as suffix tree, have close
relationship with them. Suffix tree is introduced in the next chatper.
