...
前面介绍的二叉树都是利用节点来存储信息。也可以利用边（edge）来存储。Radix树，如Trie和Patricia就是这样的数据结构。它们产生于1960年代，被广泛用于编译器\cite{okasaki-int-map}和生物信息处理（如DNA模式匹配）\cite{wiki-suffix-tree}等领域。
