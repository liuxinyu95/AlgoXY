...
整数前缀树是一种特殊的二叉树。或者为空$\nil$，或者是叶子节点$(k, v)$，包含整数键$k$和值$v$；或者是分枝节点$(p, m, l, r)$，其左右子树$l$、$r$共有一个二进制\textbf{最长公共前缀}$p$，左子树的下一位是0，而右子树的下一位是1。掩码（mask）$m$表明从哪一位开始分枝出子树。掩码为$m = 2^h$的形式，其中整数$h \geq 0$，所有低于$h$位的二进制位都不属于公共前缀。下面的例子代码定义了整数前缀树。
...
\subsection{插入}
\index{整数前缀树!插入}
当向树$T$插入整数$y$时，若$T$为空，我们用$y$创建一个叶子节点；如果$T$本身只包含一个叶子节点$x$，我们创建一个分枝，并将两个叶子节点$x$和$y$设为子树。为了确定左右，我们需要找到$x$和$y$的最长公共前缀$p$。例如，$x = 12 = (1100)_2$、$y = 15 = (1111)_2$，则$p = (11oo)_2$，其中$o$表示我们不关心的二进制位，我们使用掩码$m=2^h$来去掉这些位。在本例中，$h = 2， m = 2^2= 4 = (100)_2$，最长公共前缀$p$后面的一位代表$2^1$。$x$中这一位是0，而$y$中这一位是1。因此$x$是左子树，而$y$是右子树。如\cref{fig:int-patricia-insert-b}所示。
