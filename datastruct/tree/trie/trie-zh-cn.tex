...
由习题\ref{ex:prefix-tree-pre-order-traverse}，对于二叉前缀树，按照前序遍历可以获得字典顺序的结果。对于多叉前缀树，我们需要对子树按字典顺序遍历。如果用自平衡排序二叉树管理子树（如红黑树或AVL树），可以在线性时间内按序遍历子树（见习题\autoref{ex:bst-iterate}）。如果用散列表或者关联列表管理子树，则需要$O(n\lg n)$时间对子树排序。
}
\end{Answer}
