...
This algorithm returns the extracted maximum elements, and modifies the tournament tree in-place.
...
The behavior of \textproc{Extract-Max} is quite similiar to the pop operation for some data structures,
...
the path that the `champion team win the world cup'.
...
to be sorted, then continously extract the maximum. If we want to sort in monotonically increase order,
...
This algorithms first takes $O(N)$ time to build the tournament tree, then performs $N$ pops to select
...
It's possible to design the tournament knock out algorithm in pure functional approach. And we'll see
...
Both children are again binary tree.
...
One option is to make this team the winner, so that it will attend the further game. Actually, we can
...
\be
bulid(L) = build'(\{wrap(x) | x \in L\})
\ee

The $build'(\mathbb{T})$ function terminates when there is only one tree left in $\mathbb{T}$, which
is the champion. This is the trivial edge case. Otherwise, it group every two trees in a pair to determine
the winners. When there is odd numbers of trees, it just makes the last tree as the winner to attend the 
...
the left trees by removing the first two. Function $pair()$ is defined as the following.
...
When extract the champion (the minimum) from the tournament tree, we need examine either the left child
...
Note that this algorithm only removes the current champion without returning it. So it's neccessary to
...
With these function defined, tournament knock out sorting can be formalized by using them.

\be
sort(L) = sort'(build(L))
\ee

Where $sort'(T)$ continously pops the minimum element to form a result list
...
And the auxiliar function \verb|only|, \verb|key|, \verb|wrap| accomplished with explict infinity support are list
as the following.
...
\begin{Exercise}
  \begin{itemize}
    \item Implement the helper function \verb|leaf()|, \verb|branch|, \verb|max()| \verb|lsleaf()|, \verb|release()| to complete the imperative tournament tree program.
    \item Implement the imperative tournament tree in a programming language support GC (garbage collection).
    \item Why our tournament tree knock out sort algorithm can handle duplicated elements (elements with same value)? We say a sorting algorithm stable, if it keeps the original order of elements with same value. Is the tournament tree knock out sorting stable?
    \item Design a imperative tournament tree knock out sort algorithm, which satisfies the following:
      \begin{itemize}
...
This is the limit of comparison based sort according to \cite{TAOCP}. However, there is still room for improvement.
After sorting, there lefts a complete binary tree with all leaves and branches hold useless infinite values.
This isn't space effecient at all. Can we release the nodes when popping?
...
Heap always keeps the minimum (or the maximum) on the top, and privdes fast pop operation.
...
sort is easy and commonly used as example to teach new programmer about embedded looping.
