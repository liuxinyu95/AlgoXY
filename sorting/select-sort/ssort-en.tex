...
number 3, and do the compare, ... We repeat this process until arrived at
...
Or it will produce the updated list in reverse order. However, it's necessary to
...
Note that the it's necessary to swap the left most and right most elements before the inner loop if they are
...
list halves after such a pass, so that we can keep reduce the list until there is only one tree left.
