...
We store the sorted result in a new list, alternatively, we can change it to in-place. Let $A = [a_1, a_2, ..., a_n]$:
...
When process the $i$-th element, all elements before $i$ are sorted. We continuously insert elements till consume all the unsorted ones, as shown in figure \ref{fig:in-place-sort}.
