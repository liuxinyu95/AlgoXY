...
\Function{Sort}{$A$}
  ...
  \EndFor
  \State \Return $Next$ \Comment{Skip to rearrange $A$ through $Next$}
\EndFunction
\end{algorithmic}
...
\begin{Exercise}
\Question{For the index based list, we return the rearranged index as result. Design the algorithm to re-order the original array $A$ from the index $Next$.}
\end{Exercise}
