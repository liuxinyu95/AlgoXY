...
if it satisfies all the predication $p_1, p_2, ...$. The result is also a list. Please refer to the appendix
...
in handling some special input data by improved partition algorithm, or tries to uniforms
...
The only thing left is to concatenate the recursive sort result in front of and after the pivots list.
\end{enumerate}
...
`equal-to' operations. The basic version of ternary sort concatenates the three sub lists by 
concatenation, which are all linear time operations. They can be eliminated by using the standard
...
Below Haskell program implements this algorithm. It eliminates the intermediate variables to represent
...
recursively sorted results, it holds list of sorted sub list, and perform concatenation
finally.
...
After the left pointer $i$ meets the right pointer $j$, or they overlaps each other, we need one extra exchanging
...
case \cite{pearls}, \cite{3-way-part}. Sedgewick, Bentley and McIlroy tries different
threshold, as known as `Cut-Off', that when 
there are lesson than `Cut-Off' elements, the sort algorithm fall-back to insertion sort.
...
\item Implement the algorithm which fall-back to insertion sort when the length of list is
...
\section{Merge sort}
Although quick sort performs perfect in average cases, it can't avoid the worst case no matter what
...
Where $l_1$ is the first element of $L$, and $L'$ represents the rest elements except of $l_1$ in $L$ if $L$
isn't empty.
...
The verbose merge algorithm continuously check the element from the two input arrays, pick the smaller one
...
Note that, this performance doesn't vary in all cases, as merge sort always uniformed divides the input.
...
\begin{figure}[htbp]
 ...
 \caption{Merge without overwriting work area.}
...
original array to be sorted. we need pass the following arguments when merging: the start points and end
...
stored in the work area; As the size of area is as same as $B$, it's OK for exchange their contents;
\item All the elements in $A$ are less than any elements in $B$. In this case, the merge algorithm
continuously exchanges elements between $A$ and the work area. After all the previous $\frac{1}{4}$ 
...
The work area is in effect advances toward the end of the array, and finally moves to the right
...
the singleton array is sorted by nature. Merge a singleton array to the other is equivalent
...
\Procedure{Sort'}{$A, l, u, w$}
  ...
  \Else \Comment{Exchange all elements to the work area}
    \While{$l \leq u$}
...
algorithm reduces the unsorted part in sequence of $N/2, N/4, N/8, ...$, it takes $O(\lg N)$ steps to complete
sorting. In every step, It recursive sort half of the rest elements, and perform linear time merging.
...
some overhead has be introduced, which increase the complexity of the merge sort algorithm. However, if
...
these two sub sequences are merged to a working area. The merged result starts from beginning.
Next we repeat this step, which goes on scanning toward the center of the original sequences.
...
Such switching happens repeatedly in each rounds. Finally, we copied all elements from the 
...
for instance, as the following example Haskell code \footnote{There is a `groupBy' function provided in
...
Different from the $sort$ function, which sort a list of elements, function $sort'$ accepts a list of 
...
For the recursive case, we call a function $mergePairs$ to merge every two sub lists, then recursive call
$sort'$.
...
This version based on Okasaki presented in \cite{okasaki-book} is quite similar to the nature merge sort
...
the pair of lists on the head, and appending the merging result to the tail. This greatly simply
...
Actually, the parallel version quick sort and morege sort, don't not only distribute
the recursive sub sequences sorting into two parallel process, but divide the sequences into
$p$ sub sequences, where $p$ is the number of processors. Idealy, if we can achieve
sorting in $O(\frac{N}{p} \lg \frac{N}{p})$ time, which is linear speed up, 
we say the algorithm is parallel optimal.

However, a straightforward parallel extension to the sequential quick sort algorithm 
which sampling several pivots, divide $p$ sub sequences, and independant
recursivly sort them in parallel, isn't optimal. The bottleneck exists in the 
...
at the merge phase. Both in practice parallel merge sort and quick sort need
good designes in order to achieve the optimal speed up. Parallelism is a big and complex 
...
all programming environments adopt quick sort as the default library sorting tool. As time goes on
some environments, especially those manipulate abstract sequence which is dynamic and not based on
pure array switch to merge sort as the general purpose sorting tool. 
...
settings with arrays. It either need extra spaces for merging, which is sometimes unreasonable, for
example in embedded system with limited memory, or cause many overhead swaps by in-place workaround.
In-place merging is till an active research area.

Although the title of this chapter is `quick sort V.S. merge sort', it's not the case the one
algorithm has nothing to do with the other. Quick sort can be viewed as a optimized version of
...
Quick sort, for example, is quite easy for merging, because the all the elements in the sub 
...
quite easy to divide for merge sort no matter what concrete divide method is taken: 
...
is still a hot research area in computer science. At the time when I am writing this chapter,
