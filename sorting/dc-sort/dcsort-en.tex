...
\be
sort(L) = concats(mergePairs(group(L)))
\ee
...
sub lists into bigger one until there sub lists are merged to the only one result.
...
mergePairs(L) = \left \{
  ...
  {r@{\quad:\quad}l}
  L & |L| = 1 \\
  mergePairs(\{ merge(L_1, L_2) \} \cup mergePairs(L'')) & otherwise
  \end{array}
\right.  
\ee

When there are less than two sub lists in the list, we are done; Otherwise, we merge the first two sub lists $L_1$ and $L_2$,
and recursively merge the rest of pairs in $L''$. The merge result is again fed to the $mergePairs$. However, the type of
the result of $mergePairs$ is list of lists, so that we use $concats$ function to flatten it. $concats$ is detailed
explained in appendix A of this book.
...
concats(L) =  \left \{
  ...
  {r@{\quad:\quad}l}
  \Phi & L = \Phi \\
  concat(L_1, concats(L')) & otherwise
  \end{array}
\right.  
\ee

The $merge$ function is as same as before. The complete example Haskell program is given as below \footnote{Haskell
standard library provides `concat' function to flatten the list of lists}.
...
in every recursion. The whole list is split into $N$ singletons at the very begining, and we only merge
...
The next is to design the $sort'$ algorithm, which repeatedly merge merge pairs of sub lists until there are only
one sub list.
...
sort'(\mathbb{L}) = \left \{
  ...
  \Phi & \mathbb{L} = \Phi \\
  L_1 & \mathbb{L} = \{ L_1 \} \\
  sort'(mergeAll(\mathbb{L})) & otherwise
  \end{array}
\right.
\ee
...
For the recursive case, we call a fuction $mergeAll$ to merge every two sub lists, then recursive call
$sort'$.

\be
mergeAll(\mathbb{L}) = \left \{
  \begin{array}
  {r@{\quad:\quad}l}
  \mathbb{L} & |\mathbb{L}| \leq 1 \\
  \{ merge(L_1, L_2) \} \cup mergeAll(L') & otherwise
  \end{array}
\right.
\ee
...
\be
sort(L)= concats(mergePairs(groupBy(\lambda_{x, y} \cdot False, L)))
\ee

That instead of spanning the non-dereasing sub list as long as possible, it uses a always false predicate,
...
sort'(\mathbb{L}) = \left \{
  ...
  \Phi & \mathbb{L} = \Phi \\
  L_1 & \mathbb{L} = \{ L_1 \} \\
  sort'(fold(merge, \Phi, \mathbb{L})) & otherwise
  \end{array}
\right.
\ee
