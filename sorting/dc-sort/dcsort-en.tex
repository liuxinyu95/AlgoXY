...
Where function $f$ compares the element with the pivot (which is passed to $f$ as a parameter, so that
$f$ is in curried form, see appendix for detail. Alternative, $f$ can be a lexical closure which is in
the scope of $partition$, so that it can access the pivot in this scope.), 
...
the pair $(A, B)$ should be reprensted by a variable, for example $P$, the using access functions
to extract its first and second parts.

The example Haskell program needs be modified accordingly.
