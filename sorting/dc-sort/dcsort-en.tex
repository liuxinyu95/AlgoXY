...
\vspace{3mm}
We can build either top-down or bottom-up. For top-down, halve the list to two, recursively build two binary search trees respectively, then merge to a tree; For bottom-up, wrap each element to a singleton leaf, then pair-wise merge trees repeatedly to the final binary search tree. Both approaches depend on the tree merge, let us define it. If either tree is empty, the merge result is the other tree; otherwise, let the two trees be $(A, x, B)$ and $(C, y, D)$, if $x < y$, use $y$ to partition $B$ to two trees $B|_{<y}$ and $B|_{\geq y}$, where the one has all elements $<y$, the other has the rest; use $x$ to partition $C$ to $C|_{<x}$ and $C|_{\geq x}$. Then we can form the merge result as $((A \oplus C|_{<x}, x, B|_{<y} \oplus C|_{\geq x}), y D \oplus B|_{\geq y})$. Symmetrically, when $x \geq y$, we partition $A$ and $D$.
