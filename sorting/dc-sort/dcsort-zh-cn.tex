...
能够不断进行两两组合必须满足交换性条件：$f(x, y) = f(y, x)$，进一步可以扩展到$n$个元素。令$f(x, y) = x \oplus y$，有$x_1 \oplus x_2 \oplus \cdots\ x_n = x_1' \oplus x_2' \oplus \cdots\ x_n'$，其中$x_1', x_2', \cdots, x_n'$是$n$个元素的任意排列。令可交换二元运算$f$的单位元为$z$，定义两两叠加为：
