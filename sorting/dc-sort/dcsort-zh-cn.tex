...
K[] sort(K[] xs) {
    var ys = [[x] | x in xs]
    while length(ys) > 1 {
        append(ys, merge(ys.pop(0), ys.pop(0)))
    return if ys == [] then [] else ys.pop(0)
}

K[] merge(K[] xs, K[] ys) {
    K[] zs = []
    while xs != [] and ys !=[] {
        append(zs, if xs[0] < ys[0] then xs.pop(0) else ys.pop(0))
    }
