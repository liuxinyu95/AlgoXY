...
划分对于众多采用分治策略的排序算法很重要。我们接下来考虑如何实现原地划分。
...
图\ref{fig:partition-1-way}描述了这种一次遍历划分的方法\cite{Bentley}\cite{CLRS}。我们从左向右扫描数组。任何时候，数组都由图\ref{fig:partition-1-way} (a)所示的几部分组成：
