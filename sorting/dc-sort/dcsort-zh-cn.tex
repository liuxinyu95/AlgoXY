...
\subsection{平均情况\texorpdfstring{$\bigstar$}{★}}
\index{快速排序!平均情况分析}
...
快速排序性能优异，但在最坏情况下，性能会退化到平方级别。如果数据随机分布，出现最坏情况的概率很低。尽管如此，工程上常常采用一些方法以避免或减少出现最坏情况。

\index{快速排序!三分划分}
我们给出的划分实现\textproc{Partition}在处理大量重复元素时出现性能下降。考虑含有$n$个相等元素的特殊序列$[x, x, ..., x]$：
