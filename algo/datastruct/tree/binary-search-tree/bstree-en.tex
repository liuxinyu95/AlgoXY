...
The last kind of quering, to find the successor or predecessor of an element
...
It seems that the functional solution is hard to be found, becasue there
is no pointer like field linking to the parent node. However, If we consider
the orginal purpose of providing $succ$/$pred$ function, `to traverse all the
...
that satisifies $y > x$, there are two cases. If the node with value $x$
...
tree in purely funcitonal way is shown in this section. It's actually
...
\begin{figure}[htbp]
	...
	\includegraphics[scale=0.5]{img/del-leaf.ps} \\ 
        ...
        \caption{Delete a node from binary search tree.} \label{fig:del-cases}
       \end{center}
\end{figure}


TODO: Funtion defintion
